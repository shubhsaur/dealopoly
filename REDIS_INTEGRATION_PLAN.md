...
- [x] Replace `disconnectTimers = new Map<string, NodeJS.Timeout>()` with Redis keys
  - `redis.set("disconnect:{code}_{playerId}", "1", { ex: 300 })` instead of `setTimeout`
  - Clear timer: `redis.del("disconnect:{code}_{playerId}")`
- [x] Remove `sweepIdleRooms()` entirely — room keys auto-expire after 4 hours via TTL
- [x] Refresh TTL on every room activity: `redis.expire("room:{code}", 14400)`
- [x] Handle expiry-triggered cleanup (abandon room / convert player to bot) via polling or Upstash keyspace notifications
...
- [x] Each server instance subscribes to `room:{code}:events` channel on room attach
- [x] After `applyCommand()` / `startGame()` / any state mutation: publish update event to Redis channel instead of broadcasting directly
- [x] Each instance's subscriber receives the published event, finds its locally connected WebSocket clients for that room, and sends them the masked game state
- [x] This is the key step that enables true horizontal scaling — any instance can serve any player
- [x] Use `ioredis` with Upstash Redis URL for pub/sub (Upstash REST client does not support pub/sub)
...
- [x] Replace `hydrateRoomsFromDb()` — scan Redis for `room:*` keys instead of querying Postgres
- [x] Redis becomes the source of truth for live state; Neon Postgres stays as the audit/history log
- [x] Safety net: if Redis is cold (no keys), fall back to Neon Postgres hydration
- [x] Add startup log: `[Hydration] Restored N rooms from Redis`
...
- [x] Test reconnect flow: player disconnects → bot takes over → player reconnects → bot reverts
- [x] Test server restart with active rooms (hydration from Redis)
- [x] Test two-instance scenario locally (two processes on different ports sharing same Redis)
- [x] Handle Redis connection failures gracefully — fall back to in-memory, log a clear warning
- [x] Add Redis stats to `/api/stats` endpoint (ping latency, key count)
- [x] Remove old in-memory sweep code and dead code
- [x] Update `.env.example`, `README.md`, and `IMPLEMENTATION_PLAN.md`
...
| Phase 3 — Timers & Expiry via TTL | ✅ Done |
| Phase 4 — Pub/Sub Broadcasting | ✅ Done |
| Phase 5 — Boot Hydration from Redis | ✅ Done |
