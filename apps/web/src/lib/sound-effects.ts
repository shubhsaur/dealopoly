 ...
 * - Android Chrome/Edge: uses navigator.vibrate() (Vibration API)
 * - iOS 18+: uses the hidden <input type="checkbox" switch> Taptic Engine trick
 */
  ...
  try {
    ...
    if ("vibrate" in navigator && typeof navigator.vibrate === "function") {
      switch (type) {
        ...
        case "medium":  navigator.vibrate(28); break;
        case "heavy":   navigator.vibrate(50); break;
        case "success": navigator.vibrate([15, 50, 25]); break;
        case "warning": navigator.vibrate([30, 40, 30]); break;
      }
      return;
    }

    // ── iOS 18+ Taptic Engine (hidden <input type="checkbox" switch>) ─────
    // Apple's Taptic Engine can be triggered from web pages by programmatically
    // toggling a native switch-style checkbox. Each toggle fires one haptic tap.
    // We keep a single hidden element in the DOM and reuse it for all calls.
    triggerIOSTaptic(type);
  } catch {
    // Ignore unsupported environments
  }
}

/** Cached hidden switch element used for iOS haptics */
let _iosSwitchEl: HTMLInputElement | null = null;

function getIOSSwitchEl(): HTMLInputElement | null {
  if (typeof document === "undefined") return null;
  if (_iosSwitchEl) return _iosSwitchEl;

  // Feature-detect: only create if the browser supports switch type
  // (iOS 18+). We detect by checking that the attribute survives assignment.
  ...
  // Style it completely off-screen and invisible
...
/**
 * Fires one haptic tap on iOS by toggling the hidden switch.
 * Each toggle (false→true or true→false) produces one physical Taptic Engine pulse.
 */
function tapIOSOnce(el: HTMLInputElement): void {
  el.checked = !el.checked;
  // Dispatch a real change event so WebKit registers the interaction
  el.dispatchEvent(new Event("change", { bubbles: true }));
}

function triggerIOSTaptic(type: "light" | "medium" | "heavy" | "success" | "warning"): void {
  ...
  switch (type) {
    ...
    case "heavy":
      ...
      tapIOSOnce(el);
      setTimeout(() => { if (_iosSwitchEl) tapIOSOnce(_iosSwitchEl); }, 80);
      break;
    case "success":
      // Double tap with a slight pause — "ta-dum"
      tapIOSOnce(el);
      setTimeout(() => { if (_iosSwitchEl) tapIOSOnce(_iosSwitchEl); }, 100);
      break;
    case "warning":
      // Two rapid taps — alerting double knock
      tapIOSOnce(el);
      setTimeout(() => { if (_iosSwitchEl) tapIOSOnce(_iosSwitchEl); }, 60);
      break;
  }
}
